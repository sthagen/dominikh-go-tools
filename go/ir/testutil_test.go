...
// * builds the *ir.Package (and not its dependencies), and
// * returns the built *ir.Package and the loaded packages.Package.
//
// TODO(adonovan): factor with similar loadFile (2x) in cha/cha_test.go and vta/helpers_test.go.
func buildPackage(t testing.TB, content string, mode ir.BuilderMode) (*ir.Package, *packages.Package) {
	...
	prog, _ := irutil.Packages(ppkgs, mode)
